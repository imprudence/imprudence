...
namespace LLAvatarNameCache
{
	...
	// Manual override for display names: 0 = legacy names,
	// 1 = display name and legacy name, 2 = display name (legacy if absent)
	U32 sUseDisplayNames = 0;
...
public:
	...
	/*virtual*/ void result(const LLSD& content)
	{
		...
		LLSD::array_const_iterator it;
		for (it = agents.beginArray(); it != agents.endArray(); ++it)
		{
			const LLSD& row = *it;
