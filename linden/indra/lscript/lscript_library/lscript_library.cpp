...
// script calls for vehicle action
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetParcelMediaURL", NULL, "s", "osSetParcelMediaURL(string url)Sets Parcel Media URL\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetParcelSIPAddress", NULL, "s", "osSetParcelSIPAddress(string SIPAddress) Sets Parcel SIP Address for Voice\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osTeleportAgent", NULL, "kiivv", "osTeleportAgent(key agent, int regionX, int regionY, vector position, vector lookat)\nfor In-Grid Teleport to Grid Map Coordinates\n(AgentUUID, XXXX,YYYY,<x,y,z>,<x,y,z>)\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osTeleportAgent", NULL, "kvv", "osTeleportAgent(key agent, vector position, vector lookat)\nfor In-Region Teleport Use,\n(AgentUUID, <x,y,z>, <x,y,z>)\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osDrawLine", NULL, "sii", "osDrawLine(string drawList, int endX, int endY)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetFontName", NULL, "ss", "osSetFontName(string drawList, string fontName)Sets Current Font to one available on Server\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osGetNotecardLine", "s", "si", "string osGetNotecardLine(string name, int line)Replaces LL Dataserver event to pull notecard data,\nReads the requested line and return its data as a string\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osMatchString", "l", "ssi", "list osMatchString(string src, string pattern, int start)Return a list of matches for the pattern and its components inside the source string.\nThe pattern is a regular expression. Each match in the result is the string that matched and its position in the source.\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osNpcCreate", "k", "ssvk", "key osNpcCreate(string firstname, string lastname, LSL_Vector position, LSL_Key cloneFrom)\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osNpcMoveTo", NULL, "kv", "key osNpcMoveTo(LSL_Key npc, LSL_Vector position)\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osNpcSay", NULL, "ks", "key osNpcSay(LSL_Key npc, string message)\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osNpcRemove", NULL, "k", "key osNpcRemove(LSL_Key npc)\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osGetRegionMapTexture", "k", NULL, "key osGetRegionMapTexture(string regionName) Returns the Map Texture UUID for the regionName requested\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osGetRegionStats", "l", NULL, "List osGetRegionStats() Returns a list of float values representing a number of region statistics (21 of the values shown in the Statistics Bar of LL-based clients)\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osGetSimulatorMemory", "i", NULL, "Integer osGetSimulatorMemory() Returns the Actual Amount of RAM used by the OpenSim Instance this is requested from\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osKickAvatar", NULL, "sss", "osKickAvatar(string FirstName, string LastName, string alert) Kicks avatar from Region with an Alert Message\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetSpeed", NULL, "kf", "osSetSpeed(key AVATAR, float SpeedModifier) Changes Specified Avatar (UUID) to speed up. It multiplies the running, walking, and flying of the avatar.\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osCauseDamage", NULL, "kf", "osCauseDamage(key AVATAR, float damage) Causes damage to specified AVATAR (UUID)\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osCauseHealing", NULL, "kf", "osCauseHealing(key AVATAR, float healing)Causes Healing to specified AVATAR (UUID)\nhttp://opensimulator.org/wiki/OSSL_Implemented"));
}
